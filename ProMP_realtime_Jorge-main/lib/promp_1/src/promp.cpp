...
{
	// Condition at the LAST time step (phase=1.0), not at time index 1.
	// The original code passed 1.0 which was implicitly cast to int 1,
	// conditioning at the 2nd time step instead of the last.
	int last_step = static_cast<int>(_s / _alpha) - 1;
	condition_via_point(last_step, goal, std);
}
